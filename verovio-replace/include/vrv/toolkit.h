...
struct SoloNote
{
    ...
    Fraction beat;
    Fraction cumulative;
    float duration; // quarter note = 0.25
    int pitch; // midi pitch
    std::string noteId;
    bool on;
    
    SoloNote() : measureIndex{-1}, beat{Fraction()}, cumulative{Fraction()}, duration{-1}, pitch{0}, noteId{""}, on{1} { }
    
    SoloNote(int m, Fraction b, Fraction c, float d, int p, std::string n, bool o) {
        measureIndex = m;
        beat = b;
        cumulative = c;
        duration = d;
...
public:
    ...
    Fraction GetCumulativeFraction(int measure, Fraction pos, const std::vector<std::pair<int, std::string>> meterChanges);
        
    /**
     * Return the index of the measure a given note is in.
     *
     * @param xmlId the ID (\@xml:id) of the note element being looked for
     * @return the index of the measure the note is in (-1 if not found)
     */
    int GetMeasureIndexForNote(const std::string &xmlId);
    
    /**
     * Return the ending note id of a tie given the starting note id.
     *
     * @param xmlId the ID (\@xml:id) of the starting note of a tie
     * @return the ID (\@xml:id) of the ending note of a tie (emtpy string if not found)
     */
    std::string GetTiedPartnerForNote(const std::string &xmlId);
