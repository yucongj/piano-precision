...
Fraction Toolkit::GetClosestFraction(float num) {
    ...
    numerator = round(num * 64.);
    denominator = 64;
    int divisor = Fraction::gcd(numerator, denominator);
    numerator = round(numerator/divisor);
    denominator = round(denominator/divisor);
...
Fraction Toolkit::GetCumulativeFraction(int measure, Fraction pos, const std::vector<std::pair<int, std::string>> meterChanges) {
    ...
    while (i < meterChanges.size()) {
        if (meterChanges.at(i).first > measure) {
            stoppingIndex = i - 1;
            break;
        } else if (meterChanges.at(i).first == measure) {
            stoppingIndex = i;
            break;
        } else {
            i++;
    ...
    // adding distance between ith meter and (i+1)th meter
    Fraction result = Fraction();
    i = 0;
    while ((i + 1) <= stoppingIndex) {
        int measureCount = meterChanges.at(i+1).first - meterChanges.at(i).first;
        Fraction meter = Fraction::fromString(meterChanges.at(i).second);
        result = result + Fraction(measureCount * meter.numerator, meter.denominator);
        i++;
    }
    Fraction meter = Fraction::fromString(meterChanges.at(i).second);
    int measureCount = measure - meterChanges.at(stoppingIndex).first;
    result = result + Fraction(measureCount * meter.numerator, meter.denominator);
    return (result + pos);
}


int Toolkit::GetMeasureIndexForNote(const std::string &xmlId)
{
    Object *element = m_doc.FindDescendantByID(xmlId);
...
std::string Toolkit::GetTiedPartnerForNote(const std::string &xmlId)
{
    Object *element = m_doc.FindDescendantByID(xmlId);
    ...
    if (element->Is(NOTE)) {
        ...
        Measure *measure = vrv_cast<Measure *>(note->GetFirstAncestor(MEASURE));
        assert(measure);
        auto objects = measure->FindAllDescendantsByType(TIE);
        for (Object *object : objects) {
            Tie *tie = vrv_cast<Tie*>(object);
            Note *startNote = vrv_cast<Note*>(tie->GetStart());
            Note *endNote = vrv_cast<Note*>(tie->GetEnd());
            if (startNote == note) {
                return endNote->GetID();
            }
        }
    }
    return "";
